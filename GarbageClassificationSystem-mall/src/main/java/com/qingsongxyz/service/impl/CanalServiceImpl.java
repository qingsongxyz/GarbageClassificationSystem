...
import static com.qingsongxyz.config.rabbitmq.StorageCacheConfig.STORAGE_EXCHANGE_NAME;
import static com.qingsongxyz.config.rabbitmq.StorageCacheConfig.STORAGE_ROUTING_KEY;
            ...
            if (batchId == -1 || !ObjectUtils.isEmpty(entries)) {
                try {
                    ...
                    for (CanalEntry.Entry entry : entries) {
                        ...
                        //判断entry是否属于库存表并且类型为ROWDATA,传入消息队列
                        if ("storage".equals(tableName) && CanalEntry.EntryType.ROWDATA.equals(entryType)) {
                            //获取序列化后的数据
                            ByteString storeValue = entry.getStoreValue();
                            log.info("投放数据:{}", storeValue);

                            rabbitTemplate.convertAndSend(
                                    STORAGE_EXCHANGE_NAME,
                                    STORAGE_ROUTING_KEY,
                                    storeValue,
                                    m -> {
                    ...
                    log.error("gcs-mall——监听出现异常:{}, canal消息回滚...", e.getMessage());
